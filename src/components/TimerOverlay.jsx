  ...
  return (
    <div className="fixed inset-0 z-[150] flex flex-col overflow-y-auto bg-[#14110d] animate-in fade-in duration-300">
       <div className="sticky top-0 z-10 flex justify-end bg-gradient-to-b from-[#14110d] to-transparent px-4 py-4 sm:px-6">
         <button onClick={() => setIsTimerModalOpen(false)} className="flex items-center gap-2 rounded-full bg-white/8 px-4 py-2 text-sm font-semibold text-ink-300 transition-colors hover:text-white">Fechar</button>
       </div>
         ...
         {showTimerSetup ? (
           <div className="w-full max-w-md rounded-[2rem] border border-white/20 bg-white/10 p-5 shadow-2xl backdrop-blur-xl animate-in fade-in duration-200 sm:p-7 md:p-8">
             ...
             <div className="mb-6 space-y-3">
               ...
               <button onClick={() => {setTimerMode('pomodoro'); setTimerMax(50*60);}} className={`w-full p-4 rounded-2xl border-2 flex items-center justify-between transition-all ${timerMode === 'pomodoro' && timerMax === 50*60 ? 'border-[#1d4ed8] bg-[#1d4ed8]/20 text-white' : 'border-white/10 bg-white/5 text-ink-300 hover:bg-white/10'}`}>
               ...
               <button onClick={() => {setTimerMode('cronometro'); setTimerMax(0);}} className={`w-full p-4 rounded-2xl border-2 flex items-center justify-between transition-all ${timerMode === 'cronometro' ? 'border-[#1d4ed8] bg-[#1d4ed8]/20 text-white' : 'border-white/10 bg-white/5 text-ink-300 hover:bg-white/10'}`}>
             ...
             <div className="mb-6 flex items-center gap-3 px-2">
               <label className="flex items-center gap-2 cursor-pointer group">
                  <input type="checkbox" checked={saveAsFavorite} onChange={() => setSaveAsFavorite(!saveAsFavorite)} className="w-4 h-4 rounded text-[#1d4ed8] focus:ring-[#1d4ed8] border-ink-400 cursor-pointer" />
                  <span className="text-sm font-medium text-ink-300">Salvar como meu método padrão</span>
               </label>
             </div>
           ...
           <div className="flex flex-col items-center animate-in fade-in duration-300">
             <div className="text-[14px] font-bold text-ink-400 uppercase tracking-widest mb-4 flex items-center gap-2">
             ...
             <div className="mt-16 flex items-center gap-6 relative">
               <button onClick={() => setIsTimerRunning(!isTimerRunning)} className="w-20 h-20 bg-white hover:bg-ink-200 text-[#14110d] rounded-full flex items-center justify-center shadow-[0_0_30px_rgba(255,255,255,0.1)] transition-transform hover:scale-105">
