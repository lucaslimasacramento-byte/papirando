...
**🔖 ONDE PARAMOS (atualizado 2026-06-19):** Blocos 2–6 + passe transversal CONCLUÍDOS. **REVISÃO FINAL DE TELAS em andamento.** Decisões já aplicadas: ✅ Comunidade **FICA**; ✅ Esquadrões **SAI** (deletado + desconectado); ✅ ponto único de telas escondidas (`src/lib/launchConfig.js`) aplicado ao menu lateral **e** à busca do Header (NÃO LANÇA: Conciliador, Bem-estar, Audiolivros, Instagram, Aplicativos). **IMPLEMENTADO (Revisão Final, 2026-06-19):** ✅ #1 Planos faixas salariais · ✅ #2 Lembretes editar/excluir (HistoricoCard renderizado) · ✅ #3 Edital "Importar com IA" importa disciplinas de verdade (handleImportEditalDisciplinas) · ✅ #4 Planejamento "Gerar com IA" persiste no localStorage (follow-up: integrar ao calendário/kanban). **FALTA (precisa de decisão do usuário):** #5 Ciclos — UI de edição + marcar-concluída, e decidir se entra no menu lateral; #6 Simulados — ranking. **BLOQUEIO DE DADOS descoberto:** o painel completo já é real (loadOfficialRankingBoard lê `profiles` + `question_answers`); só o preview da sidebar (`buildRankingPreview`) é fake. Mas das 3 visões pedidas, só **média de acertos %** é viável com os dados cross-user atuais. **Nº de simulados** e **XP** NÃO existem em forma consultável por usuário (question_answers são respostas avulsas, não simulados; XP é calculado só localmente; profiles não tem coluna de XP). Pesos definidos para a visão Geral quando os dados existirem: acertos 50% / nº simulados 30% / XP 20% (cada métrica normalizada 0–100). Decisão pendente: criar backend (tabela de simulados + coluna XP em profiles) ou lançar só com as métricas reais disponíveis + consertar o preview fake.
