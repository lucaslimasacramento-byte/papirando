...
- **Migration `202607020003_topic_review_schedule.sql`:** tabela `topic_review_schedule` — um card FSRS por (user, disciplina, topico), RLS own-row, índices por user e (user, due). Idempotente. ✅ **APLICADA NO PROD (2026-07-05)** via `supabase db query --linked` — tabela confirmada com RLS ligada + 1 policy.
- **`src/lib/topicReviewApi.js`:** `ensureTopicReviewCards` (upsert com ignoreDuplicates — não reseta progresso), `getDueTopicReviews`, `countDueTopicReviews`, `submitTopicReview` (recalcula FSRS via `scheduleCard` e grava próximo due). Reusa a engine `src/lib/fsrs.js` dos flashcards. **4 testes passando** em `topicReviewApi.test.js`.
- **`studyPlanStore.approveStudyPlan`:** ao aprovar o plano, inscreve na repetição espaçada os tópicos que o plano agenda como bloco "Revisao" (conjunto intencional; fire-and-forget, não derruba a aprovação).
- **`Planejamento.jsx` — painel "Revisão espaçada" (NOVO):** aparece entre o cronograma IA e o conteúdo do modo quando há revisão vencida (`getDueTopicReviews` no mount). Cada tópico tem 4 botões de avaliação (Errei/Difícil/Lembrei/Fácil, cores semânticas `pl-*`) com preview do próximo intervalo (`formatNextInterval`); avaliar remove da fila do dia. Sem tela separada — decisão de design registrada no código.
- Compila limpo (esbuild), app sobe sem erros de console. Validação visual do painel pendente de login + migração aplicada.
- **Próximo da trilha:** #5 trilha DAG. `questoes_meta` derivado (#4-antigo).
...
- **WS5 · Personalizar o objetivo ✅:** botão de lápis no card do curso (Planos) abre modal `EditObjectiveModal` — edita **nome, tipo (concurso/vestibular/graduação/livre), data da prova e foto** + atalho "Editar matérias e tópicos" que leva ao editor de Disciplinas (add/remover já existe). Handler `updateCourse(courseId, patch)` no App.
- **Foto: upload de arquivo ✅:** `uploadUserImage(userId, file)` em `profileApi.js` (reusa os buckets do avatar — `avatars`/`profile-avatars`, prefixo `userId/`, sem alterar o avatar do perfil; valida tipo/≤5 MB). Handler `uploadCourseImage` no App → `onUploadImage` no modal. Botão "Enviar foto" com preview + fallback por URL. Não precisa de bucket/migração novos.
...
| Disciplinas | `Disciplinas.jsx` | ✅ | Regra de produto aplicada: **disciplina só existe alimentada por um objetivo do aluno**. Sem objetivo cadastrado, a página bloqueia a criação e mostra `SemObjetivoState` com CTA → aba Objetivos (botão "Nova disciplina" some). Com objetivo, o modal `EditarDisciplinaModal` deixou de aceitar "Geral": campo virou "Objetivo vinculado" (seletor dos objetivos do aluno, default no 1º) e o save valida o vínculo. `cursos` passado para Disciplinas via AppTabContent |
