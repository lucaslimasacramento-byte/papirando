...
- **Migration `202607020003_topic_review_schedule.sql`:** tabela `topic_review_schedule` — um card FSRS por (user, disciplina, topico), RLS own-row, índices por user e (user, due). Idempotente. ⚠️ **FALTA APLICAR NO PROD** — o comando foi bloqueado por permissão na sessão; rodar: `supabase db query --linked -f supabase/migrations/202607020003_topic_review_schedule.sql` (ou colar no SQL Editor). Até aplicar, o app degrada sozinho (painel não aparece; inscrição falha em silêncio com warn).
